...
"tbd" = > pkg.license;
"Two classes (Package & PackageVersion) to help create packages to be used "
"with ChuMP (the ChucK Manager of Packages)" = > pkg.description;
...
"chugins/Chumpinate/" + ver.version() + "/" + ver.os() +
    "/Chumpinate.zip" = > string path;

<<<path>>>;

// wrap up all our files into a zip file, and tell Chumpinate what URL
// this zip file will be located at.
ver.generateVersion("./", "Chumpinate_windows",
                    "https://ccrma.stanford.edu/~nshaheed/" + path);

chout <=
    "Use the following commands to upload the package to CCRMA's servers:" <=
    IO.newline();
chout <= "ssh nshaheed@ccrma-gate.stanford.edu \"mkdir -p "
         "~/Library/Web/chugins/Chumpinate/" <= ver.version() <=
    "/" <= ver.os() <= "\"" <= IO.newline();
chout <= "scp Chumpinate_windows.zip "
         "nshaheed@ccrma-gate.stanford.edu:~/Library/Web/" <= path <=
    IO.newline();

// Generate a version definition json file, stores this in
// "chumpinate/<VerNo>/Chumpinate_windows.json"
