...
TEST_CASE("latest_version returns the highest compatible version", "[latest_version]") {
    // Mockup versions
    PackageVersion v1("1.0.0", "1.5.2.1", "10.1", "linux", X86_64, {{"./","file1"}, {"./","file2"}});
    PackageVersion v2("2.0.0", "1.5.2.1", "10.1", "linux", X86_64, {{"./","file1"}, {"./","file2"}});
    PackageVersion v3("3.0.0", "1.5.2.1", "10.1", "linux", X86_64, {{"./","file1"}, {"./","file2"}});
    PackageVersion v4("3.0.1", "1.5.2.1", "1.5.2.4", "10.1", "linux", X86_64, {{"./","file1"}, {"./","file2"}});
    PackageVersion v5("3.0.2", "1.5.2.1", "1.5.2.4", "10.1", "any", ARCH_ALL, {{"./","file1"}, {"./","file2"}});
    ...
    Package package {"TestPackage", {"Author1", "Author2"}, "https://example.com", "https://github.com/example/test", "MIT", "Test package description", {"keyword1", "keyword2"}, {v1, v2, v3, v4}};
        ...
        SECTION("Test with compatible versions") {
            ...
            auto latest = package.latest_version(os, arch, language_ver, api_ver);
        ...
        SECTION("Test with language_version_max") {
            ChuckVersion language_ver = ChuckVersion("1.5.2.5 (chai)"); // Mockup ChuckVersion
            ApiVersion api_ver = ApiVersion("10.1"); // Mockup ApiVersion
            string os = "linux";
            Architecture arch = X86_64;
...
TEST_CASE("version() test") {
  ...
  SECTION("Test with compatible versions") {
    ChuckVersion language_ver = ChuckVersion("1.5.2.1 (chai)"); // Mockup ChuckVersion
    ApiVersion api_ver = ApiVersion("10.1"); // Mockup ApiVersion
    string os = "linux";
  ...
  SECTION("Test with incompatible chuck version") {
    ChuckVersion language_ver = ChuckVersion("1.5.2.0 (chai)"); // Mockup ChuckVersion
    ApiVersion api_ver = ApiVersion("10.1"); // Mockup ApiVersion
    string os = "linux";
    PackageVersion version("1.0.0");
  ...
  SECTION("Test no 'api_version'") {
    // Mockup Package with versions
    PackageVersion v1_no_api("1.0.0", "1.5.2.1", "linux", X86_64, {{"./","file1"}, {"./","file2"}});
    PackageVersion v2_no_api("2.0.0", "1.5.2.1", "linux", X86_64, {{"./","file1"}, {"./","file2"}});

    package = {
      "TestPackage",
      {"Author1", "Author2"},
      "https://example.com", "https://github.com/example/test", "MIT",
    ...
    latest = package.latest_version(os, arch, language_ver, api_ver);
    REQUIRE(latest.has_value());
    REQUIRE(*latest == v2_no_api);
  }
